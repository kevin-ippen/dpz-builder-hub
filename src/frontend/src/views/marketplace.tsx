...
export default function MarketplaceView() {
  ...
  const fetchMarketplace = useCallback(async (cat: string | null) => {
    setLoading(true);
    try {
  ...
  useEffect(() => {
    setStaticSegments([]);
    setDynamicTitle('Marketplace');
    return () => { setStaticSegments([]); setDynamicTitle(null); };
  }, [setStaticSegments, setDynamicTitle]);

  useEffect(() => {
    fetchMarketplace(categoryFilter);
  }, [categoryFilter, fetchMarketplace]);
      ...
      <div className="relative overflow-hidden rounded-xl border bg-gradient-to-br from-primary/5 via-background to-primary/10 p-8">
        <div className="relative z-10">
          ...
          <h1 className="text-3xl font-bold tracking-tight mb-2">
            Discover, adopt, and reuse certified assets
          </h1>
          <p className="text-sm text-muted-foreground max-w-xl">
            Browse production-ready components from across the organization. Every asset is versioned,
            governed, and tracked — adopt with confidence.
          </p>
          ...
          {stats && (
            <div className="flex items-center gap-8 mt-6 pt-4 border-t border-dashed">
              ...
              <StatCounter icon={Download} value={stats.total_installs} label="Installs" />
        ...
        <>
          ...
          {featured.length === 0 && trending.length === 0 && recent.length === 0 && (
            <div className="text-center py-16 space-y-3">
              ...
              <h3 className="text-lg font-semibold">No marketplace items yet</h3>
              <p className="text-sm text-muted-foreground max-w-md mx-auto">
                Assets appear here once they reach <strong>validating</strong> maturity or higher
                and have a published version. Submit an asset to get started.
              </p>
              <Button onClick={() => navigate('/submit')} className="mt-4">Submit an Asset</Button>
            </div>
          )}
        </>
      )}
